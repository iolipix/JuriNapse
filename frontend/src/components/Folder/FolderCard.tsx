...
export default function FolderCard({ folder, onOpen, isOwner }: FolderCardProps) {
  ...
  const folderPosts = userPosts.filter(post => post.folderId === (folder._id || folder.id));
    ...
    <div className="group relative bg-white rounded-lg shadow-sm border border-gray-200 hover:shadow-md transition-shadow duration-200">
      ...
      >
        ...
        <div className="flex items-center gap-4 mt-4 text-sm text-gray-500">
          ...
          <div>
            Créé le {folder.createdAt ? formatDate(folder.createdAt) : 'Date inconnue'}
          </div>
        </div>
      </div>
    </div>
