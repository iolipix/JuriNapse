...
export const SubscriptionProvider: React.FC<SubscriptionProviderProps> = ({ children }) => {
  ...
  const isFollowingSync = React.useCallback((userId: string): boolean => {
    ...
    const result = !!matchingSubscription;    return result;
  }, [user, subscriptions]);
  ...
  const getSuggestedUsers = React.useCallback(async (): Promise<User[]> => {
    ...
    try {      // Récupérer tous les utilisateurs
      ...
      const followedUserIds = new Set(subscriptions.map(sub => sub.id));
      const followedUsernames = new Set(subscriptions.map(sub => sub.username).filter(Boolean));
      const blockedUserIds = new Set(blockedUsers.map(blocked => blocked.id));
      const blockedUsernames = new Set(blockedUsers.map(blocked => blocked.username).filter(Boolean));      
