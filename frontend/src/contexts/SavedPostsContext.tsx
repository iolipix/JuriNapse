...
export const SavedPostsProvider: React.FC<SavedPostsProviderProps> = ({ children }) => {
  ...
  useEffect(() => {
    if (user?.id) {
      // Charger directement ici pour éviter les dépendances
      const loadSavedPosts = async () => {
        ...
        try {
          ...
          setError(error.response?.data?.message || 'Erreur lors du chargement des posts sauvegardés');
        } finally {
          setLoading(false);
        }
      };
  ...
  }, [user?.id]); // Utiliser seulement user.id pour éviter les boucles
