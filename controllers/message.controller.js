...
exports.getMessages = async (req, res) => {
  try {
    ...
    const messages = await Message.find(messageQuery)
      .populate('authorId', 'username firstName lastName profilePicture university isStudent')
      .populate('sharedPost.author', 'username firstName lastName profilePicture')
      .populate('sharedFolder.author', 'username firstName lastName profilePicture')
      .populate('sharedPdf.author', 'username firstName lastName profilePicture')
      .populate({
        path: 'replyTo',
        populate: {
          path: 'authorId',
    ...
    const messagesWithReactions = await enrichMessagesWithReactions(messages);

    // Traiter les messages pour remplacer les informations des utilisateurs supprimés
    const processedMessages = processMessages(messagesWithReactions);
...
exports.createMessage = async (req, res) => {
  ...
  const { groupId, content, replyToId } = req.body;
    ...
    await message.populate('authorId', 'username firstName lastName profilePicture university isStudent');
    ...
    if (io) {
      ...
      if (groupWasUpdated) {
        ...
        const updatedGroup = await Group.findById(groupId).populate('members', 'username firstName lastName profilePicture university isStudent');
        if (updatedGroup) {
...
exports.sharePost = async (req, res) => {
  try {
    ...
    // Récupérer le post
    const post = await Post.findById(postId).populate('authorId', 'username firstName lastName profilePicture');
    ...
    await message.populate('authorId', 'username firstName lastName profilePicture university isStudent');
    await message.populate('sharedPost.author', 'username firstName lastName profilePicture');
...
exports.shareFolder = async (req, res) => {
  try {
    ...
    const folder = await Folder.findById(folderId).populate('authorId', 'username firstName lastName profilePicture');
    ...
    await message.populate('authorId', 'username firstName lastName profilePicture university isStudent');
    await message.populate('sharedFolder.author', 'username firstName lastName profilePicture');
...
exports.sharePdf = async (req, res) => {
  try {
    ...
    await message.populate('authorId', 'username firstName lastName profilePicture university isStudent');
    await message.populate('sharedPdf.author', 'username firstName lastName profilePicture');
