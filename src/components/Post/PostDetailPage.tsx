...
const PostDetailPage: React.FC<PostDetailPageProps> = ({ 
  ...
  }, [postId, posts, getPostBySlugOrId]);
  ...
  // Charger les commentaires quand le post est disponible
  useEffect(() => {
    loadComments();
  }, [loadComments]);
    ...
    return (
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8 text-center">
          ...
          <h3 className="text-xl font-medium text-gray-900 mb-2">Publication introuvable</h3>
  ...
  const getRecommendedPosts = () => {
    ...
    const availablePosts = posts.filter(p => p.id !== post.id && !p.isPrivate);
    ...
    const scoredPosts = availablePosts.map(p => {
      ...
      score += Math.min(p.likes / 5, 2); // Max 2 points pour les likes
    ...
    const uniqueRecommended = recommended.filter((post, index, self) => 
            ...
            {commentsFromAPI.length > 1 && (
              <div className="relative" ref={sortDropdownRef}>
                ...
                >
                  ...
                  <span>{getSortLabel(commentSort)}</span>
        ...
        {user ? (
          <form onSubmit={handleSubmitComment} className="p-6 border-b border-gray-200">
            <div className="flex space-x-3">
              <div className="h-8 w-8 bg-gradient-to-br from-blue-100 to-indigo-100 rounded-full flex items-center justify-center flex-shrink-0">
                {user.profilePicture ? (
                  <img 
        ...
        <div className="divide-y divide-gray-200">
          {loadingComments ? (
            ...
            <div className="p-8 text-center text-gray-500">
              ...
              <p className="text-lg font-medium mb-1">Aucun commentaire</p>
            ...
            sortedComments.map((comment) => {
              ...
              return (
                <div key={commentId} className="p-6">
                  <div className="flex space-x-3">
                    <button
                      onClick={() => onViewUserProfile(author._id || author.id)}
                      className="h-8 w-8 bg-gradient-to-br from-blue-100 to-indigo-100 rounded-full flex items-center justify-center hover:from-blue-200 hover:to-indigo-200 transition-colors flex-shrink-0 overflow-hidden"
                    >
                    ...
                    <div className="flex-1">
                      ...
                      {!isEditing && (
                        <div className="flex items-center space-x-4">
                          ...
                          >
                            <Heart className={`h-4 w-4 ${isCommentLiked ? 'fill-current' : ''}`} />
      ...
      {recommendedPosts.length > 0 && (
        <div className="mb-8">
          <div className="flex items-center space-x-2 mb-6">
            ...
            <h2 className="text-xl font-bold text-gray-900">Publications recommandées</h2>
          </div>
