...
import { Plus, Users, Send, Search, MoreVertical, UserPlus, X, User, Edit, Trash2, ExternalLink, Check, EyeOff, Folder, FileText, Reply, Smile, Camera, Bell, BellOff } from 'lucide-react';
...
const MessagingPage: React.FC<MessagingPageProps> = ({ onViewPost, onViewUserProfile, targetUserId }) => {
  ...
  const canSendMessage = useMemo(() => {
    ...
    // Autoriser l'envoi dans une conversation virtuelle (temp-)
    if (activeGroupId.startsWith('temp-')) return true;
  ...
  }, [getVisibleGroups, computedLastMessages]);  const filteredChats = allChats.filter(chat => 
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    // Conversation virtuelle: créer la vraie conv avant d'envoyer
    if (activeGroupId.startsWith('temp-')) {
      try {
        ...
        const updated = getVisibleGroups();
        const real = updated.find(g => g.isPrivate && g.members.length === 2 && g.members.some((m: any) => m.id === userId));
        if (real) {
          setActiveGroupId(real.id);
          await loadMessages(real.id);
          if (replyingToMessage) {
            sendMessage(real.id, text, replyingToMessage.id);
            setReplyingToMessage(null);
          } else {
            sendMessage(real.id, text);
          }
          setMessageInput('');
          setTimeout(() => markGroupAsRead(real.id), 100);
        } else {
    ...
    // Conversation réelle
    if (replyingToMessage) {
      sendMessage(activeGroupId, text, replyingToMessage.id);
      setReplyingToMessage(null);
    } else {
      sendMessage(activeGroupId, text);
    }
    setMessageInput('');
  ...
  const handleCreatePrivateChat = async (userId: string) => {
    ...
    try {
      ...
      // Aucune conversation existante, créer d'abord une conversation virtuelle
      const virtualConversationId = `temp-${userId}-${Date.now()}`;
      setActiveGroupId(virtualConversationId);
      setShowNewChat(false);
      setShowSuccessMessage('Conversation prête ! Tapez votre message.');

      // En arrière-plan, essayer de créer la vraie conversation
      setTimeout(async () => {
        try {
          await createPrivateChat(userId);
          // Recharger les groupes et basculer vers la vraie conversation si créée
          setTimeout(async () => {
            try {
              // @ts-ignore
              await messagingContext.loadGroups();
          ...
          // On garde la conversation virtuelle, la vraie sera créée à l'envoi
        }
      }, 100);
    } catch (error: any) {
  ...
  const renderMessageContent = useCallback((message: any) => {
    ...
    if (message.sharedPost) {
      const post = posts.find(p => p.id === message.sharedPost.id);
  ...
  <div className="flex-1 flex flex-col max-w-full overflow-hidden min-h-0">
          {activeChat ? (
            <>
              {/* Header du chat */}
              <div className="p-4 border-b border-gray-200 bg-white flex-shrink-0">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    {/* Avatar cliquable pour les conversations privées */}
                    {activeChat.isPrivate ? (
                      <button
                        ...
                        className="h-10 w-10 bg-gradient-to-br from-blue-100 to-indigo-100 rounded-full flex items-center justify-center overflow-hidden hover:ring-2 hover:ring-blue-500 transition-all"
                      ...
                      <div className="h-10 w-10 bg-gradient-to-br from-blue-100 to-indigo-100 rounded-full flex items-center justify-center overflow-hidden">
                ...
                {groupMessages.length === 0 ? (
                  ...
                  <>
                    ...
                    if (!hasReactions && message.updatedAt && message.createdAt) {
                      ...
                      const timeDiff = new Date(message.updatedAt).getTime() - new Date(message.createdAt).getTime();
                    ...
                    const showTime = showAuthor || (index > 0 && new Date(message.createdAt).getTime() - new Date(previousMessage.createdAt).getTime() > 300000); // 5 minutes
              ...
              {canSendMessage ? (
                <div className="border-t border-gray-200 bg-white sticky bottom-0 left-0 right-0 z-10 flex-shrink-0">
                  ...
                  {replyingToMessage && (
                    <div className="px-4 py-2 bg-gray-50 border-b border-gray-200 flex items-center justify-between w-full overflow-hidden">
                ...
                {showMentionSuggestions && mentionSuggestions.length > 0 && activeChat && !activeChat.isPrivate && (
                  <div className="absolute bottom-full left-4 right-4 mb-2 bg-white border border-gray-200 rounded-lg shadow-lg max-h-48 overflow-y-auto z-20">
